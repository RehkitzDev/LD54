...
function odinSetupDefaultImports(wasmMemoryInterface, consoleElement) {
	...
	return {
		"env": {
			...
			"llvm.wasm.memory.atomic.wait32.i32": (n) => {
				return wasmMemoryInterface.atomicWait32(n);
			},
			"llvm.wasm.memory.atomic.notify.i32": (n) => {
				return wasmMemoryInterface.atomicNotify32(n);
			},
			...
			init_event_raw: (ep) => {
				...
				if (window.TouchEvent && e instanceof TouchEvent){
					if (e.touches.length > 0) {
						...
						wmi.storeI64(off(8), touch.screenX);
						wmi.storeI64(off(8), touch.screenY);
						wmi.storeI64(off(8), touch.clientX);
						wmi.storeI64(off(8), touch.clientY);
						wmi.storeI64(off(8), 0);
						wmi.storeI64(off(8), 0);
						wmi.storeI64(off(8), touch.pageX);
						wmi.storeI64(off(8), touch.pageY);
					}
				}
...
let audio_context;
let sound_files = [];
let sound_files_id = 0
let sounds_playings = [];
let sounds_playing_id = 0;
const audio = {
	init_audio: () =>{
		audio_context = new AudioContext();
		
		var fix_audio_context = () => {
			audio_context.resume()
			
			let buffer = audio_context.createBuffer(1, 1, 22050);
			...
			document.removeEventListener('click', fix_audio_context);
			document.removeEventListener('touchstart', fix_audio_context);
			document.removeEventListener('touchend', fix_audio_context);
			document.removeEventListener('mousedown', fix_audio_context);
			document.removeEventListener('keydown', fix_audio_context);
		}
		
		// fix_audio_context()
		document.addEventListener('click', fix_audio_context);
		document.addEventListener('touchstart', fix_audio_context);
		document.addEventListener('touchend', fix_audio_context);
		document.addEventListener('mousedown', fix_audio_context);
		document.addEventListener('keydown', fix_audio_context);
	},
	_load_sound: async (ptr, len) => {
		let data = wasm_memory_hack.loadBytes(ptr, len);
		let id = sound_files_id++

		let save = new Uint8Array(data)

		let buffer = await new Promise((resolve, reject) => {
			audio_context.decodeAudioData(save.buffer, resolve, reject);
		});
	...
	play_sound: (id, volume, pitch, looped) => {
		let sound = sound_files[id];

		if (sounds_playing_id >= Number.MAX_SAFE_INTEGER -1) {
			sounds_playing_id = 0;
		}
		...
		if (sound) {
			...
			source.start(0);
			sounds_playings.push({
				id: sounds_playing_id,
				source: source,
			});
			...
			return sounds_playing_id++;
		}
		return -1;
	},
	stop_sound: (id) => {
		let sound = sounds_playings[id];
		if (sound) {
			sound.source.stop();
		}
	},
	stop_all_sounds: () => {
		for (let i = 0; i < sounds_playings.length; i++) {
			let sound = sounds_playings[i];
			sound.source.stop();
		}
	},
	pitch_sound: (id, pitch) => {
		let sound = sounds_playings[id];
		if (sound) {
			sound.source.playbackRate.value = pitch;
		}
	},
	volume_sound: (id, volume) => {
		let sound = sounds_playings[id];
		if (sound) {
			sound.source.gain.value = volume;
		}
	}
}

// gamepad
let pad_pressed = new Array(16).fill(false);
let pad_pressed_once = new Array(16).fill(false);
let pad_axes = new Array(6).fill(0);

const gamepad = {
	get_state: (buffer_ptr) => {
		...
		if (pad) {
			...
			let axes = pad.axes;
			for (let i = 0; i < 16; i++) {
				let pressed = buttons[i].pressed;
				pad_pressed_once[i] = pressed && !pad_pressed[i];
				pad_pressed[i] = pressed;
			}
			for (let i = 0; i < 4; i++) {
				pad_axes[i] = axes[i];
			}

			pad_axes[4] = buttons[6].value;
			pad_axes[5] = buttons[7].value;
		}

		
		if (pad){
			let offset = 0;
			wasm_memory_hack.storeI8(buffer_ptr + offset, true);
			offset += 1;
			for (let i = 0; i < 16; i++) {
				wasm_memory_hack.storeI8(buffer_ptr + offset, pad_pressed[i]);
				offset += 1;
			}
			for (let i = 0; i < 16; i++) {
				wasm_memory_hack.storeI8(buffer_ptr + offset, pad_pressed_once[i]);
				offset += 1;
			}
			wasm_memory_hack.storeF32(buffer_ptr + offset, pad_axes[0]);
			offset += 4;
			wasm_memory_hack.storeF32(buffer_ptr + offset, pad_axes[1]);
			offset += 4;
			wasm_memory_hack.storeF32(buffer_ptr + offset, pad_axes[2]);
			offset += 4;
			wasm_memory_hack.storeF32(buffer_ptr + offset, pad_axes[3]);
			offset += 4;
			wasm_memory_hack.storeF32(buffer_ptr + offset, pad_axes[4]);
			offset += 4;
			wasm_memory_hack.storeF32(buffer_ptr + offset, pad_axes[5]);
			offset += 4;

			return true
		}
		wasm_memory_hack.storeI8(buffer_ptr, false);
		return false
	}
}
...
async function runWasm(wasmPath, consoleElement, extraForeignImports) {
	...
	const wasm = await instantiate(file, imports);
